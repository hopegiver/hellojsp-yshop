...
data.setTable("tb_category a JOIN tb_product b ON b.categoryid = a.id");
...
if(m.isPost() && f.validate()) {
    ...
    if(!f.get("category").equals(info.s("categoryid"))){
        productDao.item("categoryid",f.get("category"));
    }
    ...
    //blog.setDebug(out);
    if(!productDao.update("product_id = " + id)) {
...
p.setLayout("shop");
p.setBody("product/product_update");
p.setLoop("list", list);
p.setVar("info", info);
